...
/**
 * Calculates the Jaccard index for a given data. Optionally supports multiple
 * threads.
 */
public final class JaccardIndex
{
    ...
    public JaccardIndex(int k)
    {
        this(k, 1);
    }
    public JaccardIndex(int k, int threads)
    {
        _k = k;
        _threads = threads;
    }
    ...
    public final void calculate(SigmoidSigmoidPreprocessor preprocessor, String startDate, String endDate)
    {
        try
        {
            Date start = DateUtils.DATE_FORMAT.parse(startDate);
            Date end = DateUtils.DATE_FORMAT.parse(endDate);
    ...
    public final void writeToSingleFile()
    {
        ...
        try
        {
            ...
            for (StockPoint pt : pts)
                writer.write(pt.getStockSymbol() + "," + DateUtils.DATE_FORMAT.format(pt.getListedDate()) + "," + pt.getJIndex() + "\n");
        ...
        try
        {
            for (String name : _preprocessor.nameMap().keySet())
            {
                ...
                for (Date date : _dateIndex.keySet())
                {
                    StockPoint pt = _preprocessor.getStockByNameDate(name, date);
                    if (pt != null)
                        writer.write(DateUtils.DATE_FORMAT.format(pt.getListedDate()) + "," + pt.getJIndex() + "\n");
                }
